class WasmBenchmarkEngine {
  constructor(mountPoint, telemetryCallback = () => {}) {
    ...
    this.bytes = new Uint8Array([0x00,0x61,0x73,0x6d,0x01,0x00,0x00,0x00,0x01,0x07,0x01,0x60,0x01,0x7f,0x01,0x7f,0x03,0x02,0x01,0x00,0x07,0x0b,0x01,0x07,0x63,0x6f,0x6d,0x70,0x75,0x74,0x65,0x00,0x00,0x0a,0x0f,0x01,0x0d,0x01,0x01,0x7f,0x20,0x00,0x21,0x00,0x20,0x00,0x0b]);
  }
  ...
  render(message = "Ready.") {
    ...
    shell.innerHTML = '<div class="engine-top"><strong>NATIVE WASM VS JS JIT BENCHMARK</strong><span id="wasm-state">READY</span></div><div class="actions"><button id="wasm-run">RUN 5M SUM</button><button id="wasm-reset">RESET</button></div><div id="wasm-log" class="log"></div>';
    this.mount.appendChild(shell);
    this.mount.querySelector("#wasm-log").textContent = message;
  ...
  run() {
    ...
    let wasmSum = 0;
    for (let i = n; i >= 0; i--) wasmSum += this.wasmInstance.exports.compute(i);
    const wasmElapsed = performance.now() - wasmStart;
    const matched = jsSum === wasmSum;
    this.setState("FINISHED");
    log.textContent = [`[Benchmark: ${n.toLocaleString()} additions]`, "", `JavaScript JIT: ${jsElapsed.toFixed(2)} ms`, `WebAssembly execution: ${wasmElapsed.toFixed(2)} ms`, `Results: JS=${jsSum} · WASM=${wasmSum}`, `Integrity: ${matched ? "PASSED" : "FAILED"}`, "", "Measured browser timings; no synthetic speedup is applied."].join("\n");
    this.sendTelemetry({ jsLatency: `${jsElapsed.toFixed(2)} ms`, wasmLatency: `${wasmElapsed.toFixed(2)} ms`, resultMatch: matched ? "YES" : "NO", status: "NATIVE_WASM_EXECUTED" });
  }
