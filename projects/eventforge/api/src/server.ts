...
app.get("/health",async(_,res)=>{try{const db=process.env.DATABASE_URL?"postgresql":"file";if(process.env.DATABASE_URL)await loadEvents();res.json({status:"ok",service:"eventforge-api",version:"1.4",kafka:kafkaConnected?"connected":"unavailable",database:db});}catch{res.status(503).json({status:"degraded",service:"eventforge-api",kafka:kafkaConnected?"connected":"unavailable",database:"unavailable"});}});
