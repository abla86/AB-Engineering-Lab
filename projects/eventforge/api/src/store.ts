...
export async function loadEvents(): Promise<WorkEvent[]> {
  if (pool) {
    ...
    return rows.map(r => ({ id: r.id, type: r.type, occurredAt: new Date(r.occurred_at).toISOString(), source: r.source, payload: { itemId: r.item_id, title: r.title } }));
  }
  try {
    return JSON.parse(await fs.readFile(file, "utf8")) as WorkEvent[];
  } catch (error: unknown) {
