# Project Map

## AB Engineering Lab

The repository contains nine development modules organised across six engineering stages.

| Stage | Project | Area | Main technologies | Engineering role |
|---|---|---|---|---|
| 01 | Hello HTML | Web | HTML / CSS | Web foundation |
| 02 | Calculator | JavaScript | HTML / CSS / JavaScript | Interaction and logic |
| 02 | Digital Clock | JavaScript | JavaScript / Browser APIs | Dynamic rendering |
| 02 | JavaScript Counter | JavaScript | JavaScript / DOM | State and events |
| 02 | Advanced JavaScript Counter | JavaScript | JavaScript | Structured application logic |
| 03 | Todo App | Applications | JavaScript / Jest / Storage | Persistence and testing |
| 03 | Task Manager | Applications | JavaScript / CSS | Application UI and interaction |
| 04 | React Task Dashboard | Frontend | React / Vite | Component architecture |
| 05 | FastAPI Learning | Backend | Python / FastAPI / SQLAlchemy | API, persistence and backend testing |

## Project roles

### Hello HTML
The starting point for browser-based development and web structure.

### Calculator
Introduces user interaction, JavaScript functions and application logic.

### Digital Clock
Introduces dynamic rendering and browser time APIs.

### JavaScript Counter
Introduces state changes, events and DOM updates.

### Advanced JavaScript Counter
Extends the basic JavaScript work with more structured behaviour.

### Todo App
Moves toward a structured application with persistence and automated tests.

### Task Manager
Continues application-oriented frontend development and interaction.

### React Task Dashboard
Introduces component-based frontend architecture using React and Vite.

### FastAPI Learning
Introduces backend services, API endpoints, data models, persistence and tests.

## Integration status

The projects have been consolidated into one repository and presented through the Engineering Lab frontend.

They are **not all one runtime application**.

| Area | Status |
|---|---|
| Repository consolidation | Complete |
| Unified project presentation | Complete |
| Browser-based lab previews | Active |
| React frontend | Active |
| FastAPI backend module | Active |
| React → FastAPI integration | Next stage |
| Full-stack data flow | Planned |

## Portfolio value

The engineering progression is:

```text
Web fundamentals
       ↓
Programming logic
       ↓
Applications
       ↓
Component architecture
       ↓
Backend development
       ↓
Data persistence
       ↓
Testing
       ↓
CI/CD
       ↓
Full-stack integration
```

The catalogue intentionally distinguishes completed work from planned integration.